/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_exit.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: nolahmar <nolahmar@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/08 11:50:13 by nolahmar          #+#    #+#             */
/*   Updated: 2023/09/08 11:50:14 by nolahmar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */


#include "minishell.h"

int	number_of_args(char **args)
{
	int	i;

	i = 0;
	while (args[i])
		i++;
	return (i);
}

int	ft_isnum(char *num)
{
	int	i;

	i = 0;
	if (num[i] == '-')
		i++;
	while (num[i])
	{
		if (num[i] < '0' || num[i] > '9')
			return (0);
		i++;
	}
	return (1);
}

int	check_args_for_exit(char **args)
{
	if (args[0] && !args[1])
	{
		free_parser(g_data.tree);
		exit(EXIT_SUCCESS);
	}
	if (number_of_args(args) > 2)
	{
		free_parser(g_data.tree);
		printf("minishel: exit: too many arguments\n");
		g_data.exit_status = 1;
		return (1);
	}
	if (!ft_isnum(args[1]))
	{
		free_parser(g_data.tree);
		printf("exit: %s: numeric argument required\n", args[1]);
		g_data.exit_status = 255;
		exit(255);
	}
	return (0);
}

void	free_memory(t_env *env_data)
{
	t_env_elem	*tmp;
	int			i;

	i = 0;
	while (env_data->head)
	{
		free(env_data->head->key);
		free(env_data->head->value);
		tmp = env_data->head->next;
		free(env_data->head);
		env_data->head = tmp;
	}
	while (env_data->env[i])
	{
		free(env_data->env[i]);
		++i;
	}
	free(env_data->env);
	free(env_data);
}

int	ft_exit(t_cmd *cmd, t_env *env_data)
{
	free_lexer(g_data.lex);
	if (g_data.alloc)
		free_memory(env_data);
	printf("exit\n");
	if (check_args_for_exit(cmd->args))
		return (1);
	g_data.exit_status = ft_atoi(cmd->args[1]);
	free_parser(g_data.tree);
	exit(g_data.exit_status);
	return (EXIT_SUCCESS);
}
